...
public class Show_web extends Activity implements OnClickListener {
	...
	private void init() {
		...
		imageview = (ImageView)findViewById(R.id.subimage);
		imageview.setImageResource(R.drawable.sub);
		rl[0] = (RelativeLayout)findViewById(R.id.cyrcle_menu);
		rl[0].setOnClickListener(this);
		rl[1] = (RelativeLayout)findViewById(R.id.shareicon);
		rl[1].setOnClickListener(this);
		rl[1].setVisibility(View.INVISIBLE);
		rl[2] = (RelativeLayout)findViewById(R.id.telegram_icon);
		rl[2].setOnClickListener(this);
		rl[2].setVisibility(View.INVISIBLE);
		rl[3] = (RelativeLayout)findViewById(R.id.fav_icon);
		...
		if (exters != null) {
			...
			object = Newsit.array.get(position);
			URL = object.URL_LINK;
			webView.loadUrl(URL);
		}
	}
	...
	private void check_fav() {
		...
		if(source.find_fav(shared.getString("table", ""), object.ID)) {
			ImageView iv_2 = (ImageView)findViewById(R.id.fav_image);
	...
	private void exitfav() {
		...
		if(source.Update(0, shared.getString("table", ""), object.ID)) {
			ImageView iv_1 = (ImageView)findViewById(R.id.fav_image);
